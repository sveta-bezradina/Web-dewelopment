...
export const useProducts = () => {
  const [products, setProducts] = useState(() => {
    ...
    return savedProducts ? JSON.parse(savedProducts) : [
      ...
      { id: 9, name: "5 years", image: "./img/package9.jpg", price: 1600, comments: [] }
    ];
  });
  ...
  const updateProductComments = (productId, updatedComments) => {
    const updatedProducts = products.map(product =>
      product.id === productId ? { ...product, comments: updatedComments } : product
    );
  ...
  return [products, addProduct, updateProductComments];
};
